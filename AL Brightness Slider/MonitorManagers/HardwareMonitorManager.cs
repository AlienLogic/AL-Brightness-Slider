...
namespace SlimShady.MonitorManagers
{
	...
	public class HardwareMonitorManager : MonitorManager
	{
		...
		{
			Console.Out.WriteLine("Loading DDC/CI Manager");
			...
			HwmMgrWinApi.MonitorEnumProc callback = (IntPtr hMonitor, IntPtr unusedHdcMonitor, ref HwmMgrWinApi.Rect unusedLprcMonitor, int unusedDwData) =>
			{
				...
				{
					Console.Error.WriteLine("GetNumberOfPhysicalMonitorsFromHMONITOR failed; Error code: " + Marshal.GetLastWin32Error());
					return true;
				}
				...
				{
					Console.Error.WriteLine("GetPhysicalMonitorsFromHMONITOR failed; Error code: " + Marshal.GetLastWin32Error());
					return true;
				}
			...
			if (!HwmMgrWinApi.EnumDisplayMonitors(IntPtr.Zero, IntPtr.Zero, callback, 0))
				Console.Error.WriteLine("An Error occured while enumerating monitors");
		}
